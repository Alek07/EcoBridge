...
from app.api.errors.services.auth import InvalidUserException, TokenValidationException
...
from app.schemas.token import JWTCreds, JWTMeta, JWTPayload
...
class UserRoutes:
    ...
    LOGIN = "user_login_with_email_and_password"
    ME = "get_currently_authenticated_user"
...
class TestUserRoutes:
    async def test_routes_exist(self, app: FastAPI, client: AsyncClient) -> None:
        ...
        res = await client.post(app.url_path_for(UserRoutes.REGISTER), json={"new_user": new_user})
...
class TestUserRegistration:
    async def test_users_can_register_successfully(
        ...
        user_in_db = user_repo.get_user_by_email(email=EmailStr(new_user["email"]))
    ...
    async def test_users_saved_password_is_hashed_and_has_salt(
        self, app: FastAPI, client: AsyncClient, session_scope: Callable
    ) -> None:
        user_repo = UsersRepository(session_scope)
        new_user = {"email": "newuser2@test.io", "username": "newuser2", "password": "newuser2"}
...
class TestUserLogin:
    async def test_user_can_login_successfully_and_receives_valid_token(
        ...
        login_data = {
            ...
            "password": "testpassword",  # insert user's plaintext password
        }
        res = await client.post(app.url_path_for(UserRoutes.LOGIN), data=login_data)
    ...
    async def test_user_with_wrong_creds_doesnt_receive_token(
        ...
        user_data["password"] = "testpassword"  # insert user's plaintext password
...
class TestAuthTokens:
    ...
    async def test_empty_token_is_invalid(self, app: FastAPI, client: AsyncClient) -> None:
        with pytest.raises(jwt.PyJWTError):
            jwt.decode("", str(SECRET_KEY), audience=JWT_AUDIENCE, algorithms=[JWT_ALGORITHM])
    ...
    async def test_error_when_token_or_secret_is_wrong(
        ...
        with pytest.raises(TokenValidationException):
            username = auth_service.get_username_from_token(
                token=wrong_token, secret_key=str(secret)  # type: ignore
            )
...
class TestUserMe:
    async def test_authenticated_user_can_retrieve_own_data(
        ...
        res = await authorized_client.get(app.url_path_for(UserRoutes.ME))
        assert res.status_code == HTTP_200_OK
    ...
    async def test_user_cannot_access_own_data_if_not_authenticated(
        ...
        res = await client.get(app.url_path_for(UserRoutes.ME))
    ...
    async def test_user_cannot_access_own_data_with_incorrect_jwt_prefix(
        ...
        res = await client.get(
            app.url_path_for(UserRoutes.ME), headers={"Authorization": f"{jwt_prefix} {token}"}
        )
        assert res.status_code == HTTP_401_UNAUTHORIZED
